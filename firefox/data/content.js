...
function HasOwnerSelected (answerElems, ownerAnswerElem) {
	...
	if (answers[0].answer.className.indexOf(classes.acceptedAnswer) >= 0) {
		...
		var icon = answers[0].answer.querySelector(selectors.customIcon);
		icon.src = self.options.peoplecheck;
		icon.className = classes.me_icon + " " + classes.me_ownercommunity;
		icon.title = titles.ownercommunity;
	} else {
		var topAnswer = answers[0].answer;
		var topAnswerVote = topAnswer.querySelector(selectors.voteDiv);
		
		var icon = document.createElement("img");
		icon.src = self.options.people;
		icon.className = classes.me_icon + " " + classes.me_community;
		icon.title = titles.community;

		topAnswerVote.appendChild(icon);
...
function ReplaceDefaultCheckmark(ownerAnswerElem) {
	...
	var icon = document.createElement("img");
	icon.src = self.options.check;
	icon.title = title;
	icon.className = classes.me_icon + " " + classes.me_owner;

	voteElem.appendChild(icon);
}
